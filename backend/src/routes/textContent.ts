...
async function getByPageName(pageName: string) {
	try {
		...
		if (exists.length === 0 && pageName) {
			await db.any("INSERT INTO text_content (page_name, label, body, isHidden) VALUES ($1, $2, $3, $4);",[pageName, pageName, '', false]);
		}
...
async function updatePageContent(data: any) {
	try {
		...
		if (exists.length === 0 || !data["pageName"]) {
